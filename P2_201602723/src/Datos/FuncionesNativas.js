...
const Informacion_1 = require("./Informacion");
...
class FuncionesNativas {
    constructor() { }
    static CrearStringLength() {
        let respuesta = new nodoCodigo_1.nodoCodigo();
    ...
    static AgregarEntorno(e) {
        let nombre = "";
        // AGREGO LENGTH
        nombre = "length_string";
        if (e.AgregarFuncion(nombre, auxExpresion_1.auxExpresion.COMBINACION_TIPOS.INTEGER)) { // SI ES TRUE ES PORQUE NO LO AGREGO YA QUE EXISTE, MANDAR ERROR                
            Informacion_1.Informacion.ErrSemantico("No se puede Declarar 2 Funciones con el mismo nombre " + nombre, 0, 0);
        }
        // AGREGO TOUPPERCASE
        nombre = "touppercase_string";
        if (e.AgregarFuncion(nombre, auxExpresion_1.auxExpresion.COMBINACION_TIPOS.STRING)) { // SI ES TRUE ES PORQUE NO LO AGREGO YA QUE EXISTE, MANDAR ERROR                
            Informacion_1.Informacion.ErrSemantico("No se puede Declarar 2 Funciones con el mismo nombre " + nombre, 0, 0);
        }
        // AGREGO TOLOWERCASE
        ...
        if (e.AgregarFuncion(nombre, auxExpresion_1.auxExpresion.COMBINACION_TIPOS.CHAR)) { // SI ES TRUE ES PORQUE NO LO AGREGO YA QUE EXISTE, MANDAR ERROR                
            Informacion_1.Informacion.ErrSemantico("No se puede Declarar 2 Funciones con el mismo nombre " + nombre, 0, 0);
        }
    }
    static AgregarCodigo() {
    ...
    static HacerCondicion(letra, l1, l2) {
        // CONSTRUYO LA CONDICION
        let C1 = new Relacional_1.Relacional(0, 0);
        let aux1 = new nodoCodigo_1.nodoCodigo();
        aux1.cabeza = letra;
        aux1.tipo = auxExpresion_1.auxExpresion.COMBINACION_TIPOS.INTEGER;
        let aux2 = new nodoCodigo_1.nodoCodigo();
        aux2.cabeza = l1;
        aux2.tipo = auxExpresion_1.auxExpresion.COMBINACION_TIPOS.INTEGER;
        let res1 = C1.Traducir_Mayorigual(aux1, aux2); // CONDICION IZQUIERDA
        aux1 = new nodoCodigo_1.nodoCodigo();
        aux1.cabeza = letra;
        aux1.tipo = auxExpresion_1.auxExpresion.COMBINACION_TIPOS.INTEGER;
        aux2 = new nodoCodigo_1.nodoCodigo();
        aux2.cabeza = l2;
        aux2.tipo = auxExpresion_1.auxExpresion.COMBINACION_TIPOS.INTEGER;
...
exports.FuncionesNativas = FuncionesNativas;
